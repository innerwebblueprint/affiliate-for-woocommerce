	...
	class AFWC_Landing_Page {
		...
		public function __construct() {
			...
			if ( self::is_enabled() ) {
				...
				add_filter( 'afwc_get_merge_tag_value_afwc_affiliate_id', array( $this, 'get_value_of_afwc_affiliate_id' ), 10, 2 );
				add_filter( 'afwc_get_merge_tag_value_afwc_affiliate_name', array( $this, 'get_value_of_afwc_affiliate_name_merge_tag' ), 10, 2 );
			}

			add_filter( 'afwc_merge_tags', array( $this, 'add_merge_tags_for_landing_page' ), 10, 2 );
		}
		...
		public function is_singular_affiliate_landing_page() {
			...
			// Added a new check `get_post_type( get_the_ID() )` for the same logic because `is_singular()` does not work when checking post type in the archive page loop.
			// This extra check was added later in version 8.9.0 as we added merge tag support to the post excerpt and product short description.
			return (bool) apply_filters(
				'afwc_is_singular_affiliate_landing_page',
				! empty( $supported_post_types ) && is_array( $supported_post_types ) && ( is_singular( $supported_post_types ) || in_array( get_post_type( get_the_ID() ), $supported_post_types, true ) )
			);
		}
		...
		/**
		 * Add merge tags specific to landing pages.
		 *
		 * @param array $merge_tags An array of existing merge tags.
		 * @return array Modified array of merge tags including landing page specific tags.
		 */
		public function add_merge_tags_for_landing_page( $merge_tags = array() ) {
			return is_array( $merge_tags ) ? array_merge( $merge_tags, array( 'afwc_affiliate_id', 'afwc_affiliate_name' ) ) : array();
		}

		/**
		 * Get the value of the {afwc_affiliate_id} merge tag.
		 *
		 * @param string $value The value.
		 * @param array  $args An array of arguments.
		 * @return string
		 */
		public function get_value_of_afwc_affiliate_id( $value = '', $args = array() ) {
			if ( empty( $this->get_affiliate_id_by_current_singular_post() ) ) {
				return $value;
			}

			if ( ! empty( $args ) && is_array( $args ) && ! empty( $args['affiliate'] ) && $args['affiliate'] instanceof AFWC_Affiliate ) {
				return ! empty( $args['affiliate']->affiliate_id ) ? $args['affiliate']->affiliate_id : '';
			}
		...
		/**
		 * Get the value of the {afwc_affiliate_name} merge tag.
		 *
		 * @param string $value The value.
		 * @param array  $args An array of arguments.
		 *
		 * @return string
		 */
		public function get_value_of_afwc_affiliate_name_merge_tag( $value = '', $args = array() ) {
			...
			if ( ! empty( $args ) && is_array( $args ) && ! empty( $args['affiliate'] ) && $args['affiliate'] instanceof AFWC_Affiliate ) {
				return ! empty( $args['affiliate']->display_name ) ? $args['affiliate']->display_name : '';
			}
