...
if ( ! class_exists( 'Affiliate_For_WooCommerce' ) ) {
	...
	final class Affiliate_For_WooCommerce {
		...
		public function constants() {
			...
			// Set WooCommerce documentation link.
		...
		public static function get_plugin_data() {
			...
			return get_plugin_data( AFWC_PLUGIN_FILE, true, false );
		}
		...
		public static function get_products_data( $args = array() ) {
			...
			$with_total_count = ( isset( $args['with_total'] ) ) ? $args['with_total'] : true;
			...
			if ( ! empty( $products_result ) ) {
				...
				$products = array( 'rows' => $rows );

				if ( $with_total_count ) {
					$products['total_count'] = $products_total_count;
				}
		...
		public function get_minimum_tracking_datetime( $format = 'Y-m-d H:i:s', $gmt = false, $affiliate_id = 0 ) {
			...
			try {
				...
				$this->log_error( __METHOD__, ( is_callable( array( $e, 'getMessage' ) ) ) ? $e->getMessage() : '' );
				$result = '';
			}
		...
		public function save_ref_url_identifier( $user_id = 0, $identifier = '' ) {
			...
			$user_with_ref_url_id = get_users(
				array(
					'meta_key'   => 'afwc_ref_url_id', // phpcs:ignore
					'meta_value' => $identifier, // phpcs:ignore
					'number'     => 1,
					'fields'     => 'ids',
				)
			);

			$user_with_ref_url_id = ! empty( $user_with_ref_url_id ) && is_array( $user_with_ref_url_id ) ? reset( $user_with_ref_url_id ) : '';
